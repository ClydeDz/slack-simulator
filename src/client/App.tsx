...
export default function App() {
  ...
  if (isLoading || !data) return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100vh', fontFamily: 'var(--slacksim-font-body)', color: 'var(--slacksim-color-fg)' }}>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', overflow: 'hidden', fontFamily: 'var(--slacksim-font-body)' }}>
      <ControlBar />
      {activeTab === 'workspace' && <WorkspaceView />}
      {activeTab === 'admin' && <AdminView />}
      {activeTab === 'logs' && <LogsView />}
      {activeTab === 'database' && <DatabaseView />}
      {topModal && <SlackModal view={topModal.view} appId={topModal.appId} stackDepth={activeModals.length} />}
    </div>
  )
}
