...
export default function ChannelChip({ label, channel, users, apps = [], onClick, plain }: Props) {
  ...
  const resolvedHumans = channel.members.map(id => {
    const u = users.find(u => u.id === id)
    return u ? { seed: u.avatarSeed, name: u.fullName, url: u.avatarUrl as string | undefined } : null
  }).filter((m): m is { seed: string; name: string; url: string | undefined } => !!m)

  // All apps are available in every channel
  const resolvedApps = apps.map(a => ({ seed: a.botUserName, name: a.name, url: a.avatarUrl }))
  ...
  const memberCountLabel = humanCount > 0 && botCount > 0
        ...
        {plain ? label : (
          <>
            {channel.type === 'private'
              ? <svg xmlns="http://www.w3.org/2000/svg" width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><rect width="18" height="11" x="3" y="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>
              : <span style={{ fontWeight: 'var(--slacksim-font-weight-bold)', lineHeight: 1 }}>#</span>
            }
            {channel.name}
          </>
        )}
              ...
              {avatarsToShow.length > 0 && (
                <div style={{ display: 'flex' }}>
                  {avatarsToShow.map((m, idx) => (
                    <div key={m.seed} style={{ marginLeft: idx === 0 ? 0 : -6, zIndex: MAX_AVATARS - idx }}>
                      <Avatar seed={m.seed} size={22} alt={m.name} url={m.url} />
                    </div>
                  ))}
                </div>
              )}
