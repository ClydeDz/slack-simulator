...
export default function Composer({ channelId, threadTs, placeholder }: Props) {
  ...
  type SlashEntry = { command: string; usage?: string; description: string; appName: string; avatarSeed: string; avatarUrl?: string }
  const slashMatches: SlashEntry[] = slashQuery !== null
    ? apps.flatMap(a =>
        a.slashCommands
          .filter(sc => sc.command.slice(1).startsWith(slashQuery.toLowerCase()))
          .map(sc => ({ command: sc.command, usage: sc.usage, description: sc.description, appName: a.name, avatarSeed: a.botUserName, avatarUrl: a.avatarUrl }))
      )
    : []
  ...
  type MentionEntry =
    | { kind: 'user'; id: string; username: string; fullName: string; avatarSeed: string; avatarUrl?: string }
    | { kind: 'app';  id: string; username: string; fullName: string; avatarSeed: string; avatarUrl?: string }

  const mentionMatches: MentionEntry[] = mentionQuery !== null
    ? [
        ...apps
          .filter(a =>
            a.botUserName.toLowerCase().startsWith(mentionQuery!.toLowerCase()) ||
            a.name.toLowerCase().includes(mentionQuery!.toLowerCase())
          )
          .map(a => ({ kind: 'app' as const, id: a.id, username: a.botUserName, fullName: a.name, avatarSeed: a.botUserName, avatarUrl: a.avatarUrl })),
        ...users
          .filter(u =>
          ...
          .slice(0, 5)
          .map(u => ({ kind: 'user' as const, id: u.id, username: u.username, fullName: u.fullName, avatarSeed: u.avatarSeed, avatarUrl: u.avatarUrl })),
      ]
    : []

  // ── Channel mention detection ──────────────────────────────────
  const channelMatches = channelQuery !== null
    ? channels
        .filter(c => c.type !== 'im' && c.type !== 'mpim' && c.name)
        .filter(c => c.name.toLowerCase().startsWith(channelQuery.toLowerCase()))
        .slice(0, 8)
    : []

  // ── Emoji shortcode detection ──────────────────────────────────
  const emojiMap = workspace?.emojiMap ?? {}
  const emojiMatches: Array<{ name: string; char: string }> = emojiQuery !== null
  ...
  async function handleSend() {
    ...
    try {
      if (trimmed.startsWith('/')) {
        ...
        console.log('[Composer] dispatching slash command:', command, 'args:', args)
  ...
  function handleKeyDown(e: React.KeyboardEvent<HTMLTextAreaElement>) {
    ...
    if (!isSlashCommandArgs && channelQuery !== null && channelMatches.length > 0) {
      if (e.key === 'Escape') { e.preventDefault(); setChannelQuery(null); return }
    ...
    if (!isSlashCommandArgs && mentionQuery !== null && mentionMatches.length > 0) {
  ...
  useEffect(() => {
    function handler(e: MouseEvent) {
      if (!textareaRef.current?.closest('.ss-composer')?.contains(e.target as Node)) {
        setMentionQuery(null)
      }
    }
  ...
  return (
    <div className="ss-composer" style={{ padding: '0 var(--slacksim-space-5) var(--slacksim-space-4)', flexShrink: 0, position: 'relative' }}>
      ...
      {slashQuery !== null && slashMatches.length > 0 && (
        <div style={{
          ...
          {slashMatches.map(entry => (
            <button
              key={entry.command}
              onMouseDown={(e) => { e.preventDefault(); selectSlashCommand(entry.command) }}
            ...
            >
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                <span style={{ fontSize: 'var(--slacksim-font-size-md)', color: 'var(--slacksim-color-fg)' }}>
                  <span style={{ fontWeight: 'var(--slacksim-font-weight-bold)' }}>{entry.command}</span>
                  {entry.usage && (
                    <span style={{ fontWeight: 'var(--slacksim-font-weight-normal, 400)', color: 'var(--slacksim-color-fg-muted)' }}>
                      {' '}{entry.usage}
                    </span>
                  )}
                </span>
      ...
      {channelQuery !== null && channelMatches.length > 0 && (
        <div style={{
          ...
          {channelMatches.map(c => (
            <button
              key={c.id}
              onMouseDown={(e) => { e.preventDefault(); insertChannelMention(c.id, c.name) }}
              className="ss-mention-item"
              style={{
            ...
            >
              <span style={{ fontSize: 'var(--slacksim-font-size-md)', color: 'var(--slacksim-color-fg-muted)', fontWeight: 'var(--slacksim-font-weight-bold)' }}>#</span>
              <span style={{ fontWeight: 'var(--slacksim-font-weight-bold)', fontSize: 'var(--slacksim-font-size-md)', color: 'var(--slacksim-color-fg)' }}>
                {c.name}
              </span>
              <span style={{ fontSize: 'var(--slacksim-font-size-sm)', color: 'var(--slacksim-color-fg-muted)', marginLeft: 'auto' }}>
      ...
      {emojiQuery !== null && emojiMatches.length > 0 && (
        <div style={{
          ...
          {emojiMatches.map(entry => (
            <button
              key={entry.name}
              onMouseDown={(e) => { e.preventDefault(); insertEmoji(entry.name, entry.char) }}
              className="ss-mention-item"
              style={{
                display: 'flex', alignItems: 'center', gap: 'var(--slacksim-space-3)',
                width: '100%', padding: '8px 12px',
      ...
      {mentionQuery !== null && mentionMatches.length > 0 && (
        <div style={{
          ...
          {mentionMatches.map(entry => (
            <button
              key={entry.id}
              onMouseDown={(e) => { e.preventDefault(); insertMention(entry.username) }}
              className="ss-mention-item"
              style={{
                display: 'flex', alignItems: 'center', gap: 'var(--slacksim-space-2)',
                width: '100%', padding: '8px 12px',
      ...
      {sendError && (
        <div style={{
          ...
          <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true" style={{ flexShrink: 0 }}><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>
          {sendError}
        </div>
      )}
      ...
      <div style={{
            ...
            <div style={{
              ...
              {isIm ? (
                ...
                <>
                  <span>Message</span>
                  {isPrivateChannel
                    ? <svg xmlns="http://www.w3.org/2000/svg" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><rect width="18" height="11" x="3" y="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>
                    : <span style={{ fontWeight: 'var(--slacksim-font-weight-bold)' }}>#</span>
                  }
        ...
        <button
          ...
          style={{
            background: canSend ? 'var(--slacksim-color-primary)' : 'var(--slacksim-color-primary-disabled)',
