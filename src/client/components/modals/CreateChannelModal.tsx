...
export default function CreateChannelModal({ onClose }: Props) {
  ...
  const [selectedMembers, setSelectedMembers] = useState<string[]>(users.map(u => u.id))
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const channel = await controlApi.createChannel(name.trim(), selectedMembers, isPrivate)
  ...
  return (
    ...
    >
      <div style={{
        ...
        {/* Modal body */}
        <form onSubmit={handleSubmit} style={{ flex: 1, overflowY: 'auto', padding: 'var(--slacksim-space-5)' }}>
          {/* Channel name */}
          <div style={{ marginBottom: 'var(--slacksim-space-5)' }}>
            ...
            <div style={{ marginTop: 'var(--slacksim-space-1)', fontSize: 'var(--slacksim-font-size-sm)', color: 'var(--slacksim-color-fg-muted)' }}>
              ...
              {users.map((user, i) => (
                <label
                  ...
                  style={{
                    ...
                    padding: '10px 12px',
                    cursor: 'pointer',
                    borderTop: i > 0 ? '1px solid var(--slacksim-color-border)' : 'none',
                    background: selectedMembers.includes(user.id) ? 'var(--slacksim-color-bg-secondary)' : 'transparent',
                  }}
                >
                  <input
                    type="checkbox"
          ...
          <div style={{ marginBottom: 'var(--slacksim-space-5)' }}>
            <label style={{
              ...
              />
              <div>
                <div style={{ fontWeight: 'var(--slacksim-font-weight-bold)', fontSize: 'var(--slacksim-font-size-md)', color: 'var(--slacksim-color-fg)' }}>
                  Make private
                </div>
                <div style={{ fontSize: 'var(--slacksim-font-size-sm)', color: 'var(--slacksim-color-fg-muted)' }}>
          ...
          {/* Actions */}
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 'var(--slacksim-space-3)' }}>
            <button
              type="button"
              onClick={onClose}
              style={{
            ...
            <button
              ...
              style={{
                background: name.trim() ? 'var(--slacksim-color-primary)' : 'var(--slacksim-color-primary-disabled)',
