...
export default function ThreadPane() {
  const { activeChannelId, activeThreadTs, setActiveThread, messages, setChannelMessages } = useStore()
    ...
    queryFn: async () => {
      ...
      if (result.ok && result.messages) {
        const threadMessages: Message[] = result.messages.map((m: any) => ({
          ...
          reactions: (m.reactions ?? []).map((r: any) => ({ name: r.name, users: r.users })),
        }))
  ...
  return (
    <div style={{
      ...
      {/* Thread messages */}
      <div style={{ flex: 1, overflowY: 'auto', padding: 'var(--slacksim-space-4) 0' }}>
        {isLoading && threadMessages.length === 0 ? (
          <div style={{
            display: 'flex', alignItems: 'center', justifyContent: 'center',
            height: '100%', color: 'var(--slacksim-color-fg-muted)',
            fontSize: 'var(--slacksim-font-size-md)',
          }}>
            Loading thread…
          </div>
              ...
              <React.Fragment key={msg.ts}>
                {showSeparator && (
                  <div style={{
                    ...
                    <div style={{ flex: 1, height: 1, background: 'var(--slacksim-color-divider)' }} />
                  </div>
                )}
