...
import type { User, Channel, Workspace, App, Message, WsEvent, LogEntry, ModalView } from '@shared/types'
...
interface WorkspaceState {
  ...
  // Actions
  setWorkspaceData: (data: { workspace: Workspace; users: User[]; channels: Channel[]; apps: App[] }) => void
...
export const useStore = create<WorkspaceState>((set, get) => ({
  ...
  setWorkspaceData: ({ workspace, users, channels, apps }) => {
    set({ workspace, users, channels, apps, actingUserId: users[0]?.id ?? 'U001', activeChannelId: channels[0]?.id ?? null })
  },
  ...
  setChannelMessages: (channelId, messages) => set(s => ({ messages: { ...s.messages, [channelId]: messages } })),

  addChannel: (channel) => set(s => ({ channels: [...s.channels, channel] })),

  openModal: (view, appId) => set(s => ({ activeModals: [...s.activeModals, { view, appId }] })),
  closeModal: () => set(s => ({ activeModals: s.activeModals.slice(0, -1) })),
  closeAllModals: () => set({ activeModals: [] }),
  updateTopModal: (view) => set(s => {
  ...
  updateAppConfig: (app) => set(s => ({ apps: s.apps.map(a => a.id === app.id ? app : a) })),
    ...
    switch (event.type) {
      case 'message_new': {
        ...
        if (!message.threadTs) {
          set(st => {
            ...
            const unreadCounts = (isDm && !isActive && !isFromActingUser)
              ? { ...st.unreadCounts, [message.channel]: (st.unreadCounts[message.channel] ?? 0) + 1 }
              : st.unreadCounts
            return {
              messages: {
                ...st.messages,
                [message.channel]: [...(st.messages[message.channel] ?? []), message],
              },
              unreadCounts,
            }
          })
      ...
      case 'reaction_updated': {
        ...
        set(st => ({
          messages: {
            ...st.messages,
            [channelId]: (st.messages[channelId] ?? []).map(m =>
      ...
      case 'channel_updated':
        set(st => ({ channels: st.channels.map(c => c.id === event.channel.id ? event.channel : c) }))
        break
      case 'log_entry':
        get().addLog(event.entry)
        break
      case 'workspace_reset':
        set({ messages: {}, unreadCounts: {}, activeChannelId: null, activeThreadTs: null, resetAt: Date.now() })
        // Refetch workspace data
        fetch('/_control/workspace').then(r => r.json()).then(data => {
      ...
      case 'message_deleted': {
        ...
        set(st => ({
          messages: {
            ...st.messages,
            [channelId]: (st.messages[channelId] ?? []).filter(m => m.ts !== ts),
          },
        }))
        break
      }
      case 'modal_open':
        set(st => ({ activeModals: [...st.activeModals, { view: event.view, appId: event.appId }] }))
        break
      case 'modal_push':
        set(st => ({ activeModals: [...st.activeModals, { view: event.view, appId: event.appId }] }))
        break
      case 'modal_update':
        set(st => {
          const idx = st.activeModals.findIndex(m => m.view.id === event.view.id)
          if (idx < 0) return {}
        ...
        break
      case 'modal_close':
        set(st => ({ activeModals: st.activeModals.filter(m => m.view.id !== event.viewId) }))
        break
    }
  },
}))
