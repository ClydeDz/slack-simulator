...
function runMigrations(db: Database.Database): void {
  ...
  try { db.exec('ALTER TABLE apps ADD COLUMN description TEXT') } catch (_) {}
  try { db.exec('ALTER TABLE apps ADD COLUMN slash_commands TEXT') } catch (_) {}
  try { db.exec('ALTER TABLE apps ADD COLUMN avatar_url TEXT') } catch (_) {}
  try { db.exec('ALTER TABLE messages ADD COLUMN blocks TEXT') } catch (_) {}
  try { db.exec('ALTER TABLE messages ADD COLUMN subtype TEXT') } catch (_) {}
  try { db.exec('ALTER TABLE users ADD COLUMN avatar_url TEXT') } catch (_) {}
  try { db.exec('ALTER TABLE workspace ADD COLUMN avatar_url TEXT') } catch (_) {}
  try { db.exec('ALTER TABLE workspace ADD COLUMN emoji_set TEXT') } catch (_) {}
  try { db.exec('ALTER TABLE apps ADD COLUMN incoming_webhooks TEXT') } catch (_) {}
  try { db.exec('ALTER TABLE messages ADD COLUMN pinned INTEGER NOT NULL DEFAULT 0') } catch (_) {}
  try { db.exec('ALTER TABLE channels ADD COLUMN archived INTEGER NOT NULL DEFAULT 0') } catch (_) {}
  try { db.exec('ALTER TABLE messages ADD COLUMN ephemeral_recipient TEXT') } catch (_) {}
  try { db.exec('ALTER TABLE messages ADD COLUMN unfurls TEXT') } catch (_) {}
  try { db.exec('ALTER TABLE apps ADD COLUMN unfurl_domains TEXT') } catch (_) {}
  try { db.exec('ALTER TABLE messages ADD COLUMN app_id TEXT') } catch (_) {}
}

export function getAllUsers(): User[] {
  const db = getDb()
  const rows = db.prepare('SELECT * FROM users').all() as Array<{
    id: string; username: string; full_name: string; email: string; avatar_seed: string; avatar_url: string | null
  }>
  return rows.map(r => ({
    id: r.id,
    username: r.username,
...
export function getChannel(id: string): Channel | null {
  ...
  const row = db.prepare('SELECT * FROM channels WHERE id = ?').get(id) as
    { id: string; name: string | null; type: string; archived: number } | undefined
  if (!row) return null

  const memberRows = db.prepare('SELECT user_id FROM channel_members WHERE channel_id = ?').all(id) as
...
function getReactionsForMessage(db: Database.Database, messageId: string): Reaction[] {
...
function rowToMessage(db: Database.Database, row: {
  id: string; channel_id: string; user_id: string; text: string; ts: string; thread_ts: string | null; blocks?: string | null; subtype?: string | null; pinned?: number | null; ephemeral_recipient?: string | null; unfurls?: string | null; app_id?: string | null
}): Message {
  ...
  return {
    ...
    unfurls: row.unfurls ? (() => { try { return JSON.parse(row.unfurls!) } catch { return undefined } })() : undefined,
    replyCount,
    replyUsers,
    latestReplyTs,
  }
...
export function getChannelMessages(
  ...
  let rows: Array<{ id: string; channel_id: string; user_id: string; text: string; ts: string; thread_ts: string | null }>
...
export function getThreadMessages(channelId: string, threadTs: string): Message[] {
  const db = getDb()
  // Get parent
  const parent = db.prepare('SELECT * FROM messages WHERE channel_id = ? AND ts = ?').get(channelId, threadTs) as
  ...
  const replies = db.prepare(`
    ...
    ORDER BY ts ASC
  `).all(channelId, threadTs) as Array<{ id: string; channel_id: string; user_id: string; text: string; ts: string; thread_ts: string | null }>
...
export function insertMessage(msg: {
  id: string; channelId: string; userId: string; text: string; ts: string; threadTs?: string; blocks?: unknown[]; subtype?: string; ephemeralRecipient?: string; appId?: string
}): Message {
  const db = getDb()
  ...
  db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(msg.id, msg.channelId, msg.userId, msg.text, msg.ts, msg.threadTs ?? null, blocksJson, msg.subtype ?? null, msg.ephemeralRecipient ?? null, msg.appId ?? null)
...
export function updateMessage(channelId: string, ts: string, text: string, blocks?: unknown[]): Message | null {
  const db = getDb()
  if (blocks !== undefined) {
    db.prepare('UPDATE messages SET text = ?, blocks = ? WHERE channel_id = ? AND ts = ?')
      .run(text, JSON.stringify(blocks), channelId, ts)
  } else {
    db.prepare('UPDATE messages SET text = ? WHERE channel_id = ? AND ts = ?').run(text, channelId, ts)
  }
  const row = db.prepare('SELECT * FROM messages WHERE channel_id = ? AND ts = ?').get(channelId, ts) as
    { id: string; channel_id: string; user_id: string; text: string; ts: string; thread_ts: string | null } | undefined
  if (!row) return null
...
export function deleteMessage(channelId: string, ts: string): boolean {
  const db = getDb()
  const result = db.prepare('DELETE FROM messages WHERE channel_id = ? AND ts = ?').run(channelId, ts)
...
export function toggleReaction(messageId: string, reactionName: string, userId: string): 'added' | 'removed' {
  const db = getDb()
  ...
  if (existing) {
    db.prepare('DELETE FROM reactions WHERE message_id = ? AND name = ? AND user_id = ?')
      .run(messageId, reactionName, userId)
    return 'removed'
  } else {
    db.prepare('INSERT OR IGNORE INTO reactions (message_id, name, user_id) VALUES (?, ?, ?)')
      .run(messageId, reactionName, userId)
    return 'added'
  }
}
...
type AppRow = {
  id: string; name: string; bot_user_id: string; bot_user_name: string;
  bot_token: string; app_token: string; signing_secret: string;
  request_url: string; subscribed_events: string; socket_mode_enabled: number;
  description: string | null; slash_commands: string | null; avatar_url: string | null;
  incoming_webhooks: string | null; unfurl_domains: string | null
}
...
function rowToApp(r: AppRow): App {
  return {
    ...
    subscribedEvents: JSON.parse(r.subscribed_events),
    socketModeEnabled: r.socket_mode_enabled === 1,
    description: r.description ?? undefined,
    avatarUrl: r.avatar_url ?? undefined,
    slashCommands: r.slash_commands ? JSON.parse(r.slash_commands) : [],
    incomingWebhooks: r.incoming_webhooks ? JSON.parse(r.incoming_webhooks) : [],
...
export function setPinned(messageId: string, pinned: boolean): Message | null {
  ...
  const row = db.prepare('SELECT * FROM messages WHERE id = ?').get(messageId) as
    { id: string; channel_id: string; user_id: string; text: string; ts: string; thread_ts: string | null; blocks: string | null; subtype: string | null; pinned: number } | undefined
  if (!row) return null
  return rowToMessage(db, row)
}

export function setMessageUnfurls(channelId: string, ts: string, unfurls: Record<string, unknown>): Message | null {
  const db = getDb()
  db.prepare('UPDATE messages SET unfurls = ? WHERE channel_id = ? AND ts = ?')
...
export function getAppByToken(token: string): App | null {
  const db = getDb()
  const row = db.prepare('SELECT * FROM apps WHERE bot_token = ? OR app_token = ?').get(token, token) as AppRow | undefined
  if (!row) return null
  return rowToApp(row)
}

export function getAppByWebhookToken(token: string): { app: App; channelId: string } | null {
  const db = getDb()
  const rows = db.prepare('SELECT * FROM apps WHERE incoming_webhooks IS NOT NULL').all() as AppRow[]
  for (const row of rows) {
    const webhooks: Array<{ channelId: string; token: string }> = row.incoming_webhooks
      ? JSON.parse(row.incoming_webhooks)
      : []
...
export function updateApp(id: string, fields: {
  requestUrl?: string
  ...
  if (fields.requestUrl !== undefined) {
    db.prepare('UPDATE apps SET request_url = ? WHERE id = ?').run(fields.requestUrl, id)
  }
  if (fields.socketModeEnabled !== undefined) {
    db.prepare('UPDATE apps SET socket_mode_enabled = ? WHERE id = ?')
      .run(fields.socketModeEnabled ? 1 : 0, id)
  }
  if (fields.subscribedEvents !== undefined) {
    db.prepare('UPDATE apps SET subscribed_events = ? WHERE id = ?')
      .run(JSON.stringify(fields.subscribedEvents), id)
  }
}
