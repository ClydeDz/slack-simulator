...
function addLog(entry: LogEntry) {
  ...
  import('./realtime').then(({ broadcast }) => {
...
export async function dispatchEvent(
  ...
  const subscribedApps = apps.filter(app =>
    app.subscribedEvents.includes(eventType) || app.subscribedEvents.includes('*')
  )

  await Promise.all(subscribedApps.map(async app => {
    ...
    const logBase: Omit<LogEntry, 'transport' | 'status' | 'durationMs' | 'error'> = {
      id: eventId,
      ts: Date.now(),
      appId: app.id,
      appName: app.name,
    ...
    if (app.socketModeEnabled) {
      ...
      addLog({ ...logBase, transport: 'socket_mode', status: acked ? 200 : 0, error: acked ? undefined : 'no_ack' })
