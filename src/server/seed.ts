...
interface SeedUser {
  id: string; username: string; fullName: string; email: string; avatarSeed: string; avatarUrl?: string
}
interface SeedChannel {
  id: string; name: string; type: string; members: string[]
}
interface SeedDm {
...
interface SeedThreadReply {
  user: string; text: string; ts_offset: number
}
interface SeedMessage {
  id: string; channel: string; user: string; text: string; ts_offset: number
...
interface SeedApp {
  ...
  description?: string
  avatarUrl?: string
  slashCommands?: Array<{ command: string; description: string; usage?: string }>
  incomingWebhooks?: string[]
  unfurlDomains?: string[]
}
interface SeedData {
  workspace: { id: string; name: string; domain: string; avatarUrl?: string; supportedEmojis?: Array<{ name: string; emoji: string }> }
  users: SeedUser[]
...
export async function seedDatabase(): Promise<void> {
  ...
  const appsData: { apps: SeedApp[] } = JSON.parse(fs.readFileSync(appsPath, 'utf8'))
  ...
  const seed = db.transaction(() => {
    ...
    const emojiSet = seedData.workspace.supportedEmojis?.length
      ? JSON.stringify(Object.fromEntries(seedData.workspace.supportedEmojis.map(e => [e.name, e.emoji])))
      : null
    db.prepare('INSERT OR REPLACE INTO workspace (id, name, domain, avatar_url, emoji_set) VALUES (?, ?, ?, ?, ?)')
      .run(seedData.workspace.id, seedData.workspace.name, seedData.workspace.domain, seedData.workspace.avatarUrl ?? null, emojiSet)

    // Users
    for (const u of seedData.users) {
      db.prepare('INSERT OR REPLACE INTO users (id, username, full_name, email, avatar_seed, avatar_url) VALUES (?, ?, ?, ?, ?, ?)')
        .run(u.id, u.username, u.fullName, u.email, u.avatarSeed, u.avatarUrl ?? null)
    }

    // Channels (public/private)
    for (const c of seedData.channels) {
      db.prepare('INSERT OR REPLACE INTO channels (id, name, type) VALUES (?, ?, ?)')
    ...
    for (let i = 0; i < seedData.messages.length; i++) {
      ...
      db.prepare('INSERT OR REPLACE INTO messages (id, channel_id, user_id, text, ts, thread_ts) VALUES (?, ?, ?, ?, ?, ?)')
      ...
      if (m.thread_reply) {
        ...
        db.prepare('INSERT OR REPLACE INTO messages (id, channel_id, user_id, text, ts, thread_ts) VALUES (?, ?, ?, ?, ?, ?)')
          .run(replyId, m.channel, m.thread_reply.user, m.thread_reply.text, replyTs, ts)
      }
    }
    ...
    for (const app of appsData.apps) {
      ...
      db.prepare(`
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `).run(
        app.id, app.name, app.botUserId, app.botUserName,
        app.botToken, app.appToken, app.signingSecret, app.requestUrl ?? null,
        JSON.stringify(app.subscribedEvents ?? []), app.socketModeEnabled ? 1 : 0,
