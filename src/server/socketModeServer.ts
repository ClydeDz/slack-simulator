...
function redeemTicket(ticket: string): string | null {
  ...
  if (Date.now() > entry.expiresAt) { _tickets.delete(ticket); return null }
...
type AckResolve = (result: { acked: boolean; responsePayload?: unknown }) => void
...
export function initSocketModeServer(httpServer: any): void {
  ...
  _wss.on('connection', (ws: WebSocket, req: IncomingMessage) => {
    ...
    ws.send(JSON.stringify({
      type: 'hello',
      num_connections: 1,
      debug_info: { host: 'slack-simulator', started: new Date().toISOString(), build_number: 1 },
    }))
...
export async function enqueueSocketModeInteractive(
  ...
  return new Promise<{ acked: boolean; responsePayload?: unknown }>((resolve) => {
    const timer = setTimeout(() => {
      _pendingAcks.delete(envelopeId)
      resolve({ acked: false })
    }, ACK_TIMEOUT_MS)
