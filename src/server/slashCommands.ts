...
function createResponseToken(
  ...
  _responseTokens.set(token, {
    channelId,
    userId,
    appBotUserId,
    appId,
    expiresAt: Date.now() + RESPONSE_URL_TTL,
  });
  return token;
}
...
export async function dispatchSlashCommand(params: {
  ...
  if (params.app.socketModeEnabled) {
    const { enqueueSocketModeSlashCommand } =
      await import('./socketModeServer');
    const envelopeId = `Ev${Date.now()}${Math.random().toString(36).slice(2, 8)}`;
    const result = await enqueueSocketModeSlashCommand(
      params.app.id,
      envelopeId,
      payload
    );
    ...
    try {
      const res = await fetch(params.app.requestUrl, {
        method: 'POST',
        headers,
        body,
      });
  ...
  app.post('/_response_url/:token', async (req, reply) => {
    ...
    const blocks = Array.isArray(body.blocks)
      ...
      : typeof body.blocks === 'string'
        ? (() => {
            try {
              return JSON.parse(body.blocks as string);
            } catch {
              return undefined;
            }
          })()
