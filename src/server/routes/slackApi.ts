...
import {
  ...
  insertMessage, updateMessage, deleteMessage, getMessageByTs,
  toggleReaction, getApps, getAppByToken, generateTs, generateId, setMessageUnfurls,
} from '../db'
import { broadcast } from '../realtime'
import { logInbound, dispatchEvent } from '../dispatcher'
import { issueTicket } from '../socketModeServer'
import { redeemTrigger, openModal, updateModal, pushModal } from '../interactivity'
...
async function handleSlackMethod(
  ...
  switch (method) {
    ...
    case 'users.list': {
      ...
      return reply.send({ ok: true, members, response_metadata: { next_cursor: '' } })
    }
    ...
    case 'conversations.list': {
      const channels = db.prepare('SELECT * FROM channels').all() as
      ...
      return reply.send({ ok: true, channels: result, response_metadata: { next_cursor: '' } })
    }
    ...
    case 'conversations.history': {
      ...
      if (!channelId) return reply.send({ ok: false, error: 'invalid_arguments' })

      const { messages, nextCursor } = getChannelMessages(channelId, limit, cursor)
    ...
    case 'conversations.replies': {
      ...
      if (!channelId || !ts) return reply.send({ ok: false, error: 'invalid_arguments' })
    ...
    case 'conversations.create': {
      ...
      db.prepare('INSERT INTO channels (id, name, type) VALUES (?, ?, ?)')
        .run(id, name, isPrivate ? 'private' : 'public')
    ...
    case 'conversations.members': {
      const channelId = body.channel as string
      if (!channelId) return reply.send({ ok: false, error: 'invalid_arguments' })
      const rows = db.prepare('SELECT user_id FROM channel_members WHERE channel_id = ?').all(channelId) as
        Array<{ user_id: string }>
      return reply.send({ ok: true, members: rows.map(r => r.user_id), response_metadata: { next_cursor: '' } })
    }
    ...
    case 'conversations.open': {
      ...
      if (!userIds.length) return reply.send({ ok: false, error: 'invalid_arguments' })
      ...
      const allIms = db.prepare('SELECT id FROM channels WHERE type = ?').all('im') as
        Array<{ id: string }>

      for (const im of allIms) {
        const members = (db.prepare('SELECT user_id FROM channel_members WHERE channel_id = ?')
          .all(im.id) as Array<{ user_id: string }>).map(r => r.user_id)
    ...
    case 'conversations.info': {
      ...
      if (!channel) return reply.send({ ok: false, error: 'channel_not_found' })
      return reply.send({
        ok: true,
        channel: {
          id: channel.id,
    ...
    case 'users.conversations': {
      ...
      const rows = db.prepare(`
        ...
        WHERE cm.user_id = ?
      `).all(userId) as Array<{ id: string; name: string | null; type: string }>
    ...
    case 'chat.postMessage': {
      ...
      if (/^U\w+$/.test(channelId)) {
        ...
        const allIms = db.prepare('SELECT id FROM channels WHERE type = ?').all('im') as Array<{ id: string }>
        let dmId: string | null = null
        for (const im of allIms) {
          const members = (db.prepare('SELECT user_id FROM channel_members WHERE channel_id = ?')
            .all(im.id) as Array<{ user_id: string }>).map(r => r.user_id)
          if ([...members].sort().join(',') === [...allMemberIds].sort().join(',')) {
            dmId = im.id
            break
          }
        }
        if (!dmId) {
          dmId = generateId('D')
          db.prepare('INSERT INTO channels (id, name, type) VALUES (?, ?, ?)').run(dmId, null, 'im')
          for (const uid of allMemberIds) {
            db.prepare('INSERT OR IGNORE INTO channel_members (channel_id, user_id) VALUES (?, ?)').run(dmId, uid)
          }
      ...
      if (targetChannel?.archived) return reply.send({ ok: false, error: 'is_archived' })
      ...
      const postChannelType = !postChannel ? 'channel' : postChannel.type === 'im' ? 'im' : postChannel.type === 'private' ? 'group' : 'channel'
      let postParentUserId: string | undefined
      if (threadTs) {
        const parentRow = db.prepare('SELECT user_id FROM messages WHERE channel_id = ? AND ts = ?').get(channelId, threadTs) as { user_id: string } | undefined
      ...
      for (const a of allApps) {
        if (text.includes(`@${a.botUserName}`) || text.includes(`<@${a.botUserId}>`)) {
    ...
    case 'chat.update': {
      ...
      if (!message) return reply.send({ ok: false, error: 'message_not_found' })
    ...
    case 'chat.delete': {
      ...
      if (!deleted) return reply.send({ ok: false, error: 'message_not_found' })
    ...
    case 'chat.postEphemeral': {
      ...
      const message = insertMessage({ id, channelId, userId: authApp.botUserId, text, ts, threadTs, blocks, subtype: 'ephemeral', ephemeralRecipient, appId: authApp.id })
      broadcast({ type: 'message_new', message })
    ...
    case 'chat.unfurl': {
      ...
      if (typeof rawUnfurls === 'string') {
        try { unfurls = JSON.parse(rawUnfurls) } catch { /* invalid */ }
      } else if (rawUnfurls && typeof rawUnfurls === 'object') {
        unfurls = rawUnfurls as Record<string, unknown>
      }

      if (!channelId || !ts || !unfurls) return reply.send({ ok: false, error: 'invalid_arguments' })

      const updated = setMessageUnfurls(channelId, ts, unfurls)
      if (!updated) return reply.send({ ok: false, error: 'message_not_found' })
    ...
    case 'chat.getPermalink': {
      ...
      if (!channelId || !messageTs) return reply.send({ ok: false, error: 'invalid_arguments' })
    ...
    case 'reactions.add': {
      ...
      if (!channelId || !name || !timestamp) return reply.send({ ok: false, error: 'invalid_arguments' })
    ...
    case 'reactions.get': {
      ...
      if (!channelId || !timestamp) return reply.send({ ok: false, error: 'invalid_arguments' })
    ...
    case 'views.open': {
      ...
      if (!triggerId || !rawView) return reply.send({ ok: false, error: 'invalid_arguments' })
      ...
      if (typeof rawView === 'string') {
        try { viewObj = JSON.parse(rawView) } catch { return reply.send({ ok: false, error: 'invalid_arguments' }) }
      } else {
        viewObj = rawView
      }

      const appIdFromTrigger = redeemTrigger(triggerId)
      if (!appIdFromTrigger) return reply.send({ ok: false, error: 'expired_trigger_id' })
    ...
    case 'views.update': {
      ...
      const rawView = body.view as Record<string, unknown> | string | undefined
      if (!viewId || !rawView) return reply.send({ ok: false, error: 'invalid_arguments' })
