...
import { getAppByWebhookToken, insertMessage, generateId, generateTs, getChannel, getApps } from '../db'
...
export async function registerHooks(app: FastifyInstance): Promise<void> {
  app.post<{ Params: { token: string } }>('/hooks/:token', async (req, reply) => {
    ...
    const hookChannelType = channel.type === 'im' ? 'im' : channel.type === 'private' ? 'group' : 'channel'
