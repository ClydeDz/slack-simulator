...
async function scrapeOgPreview(
  url: string,
...
export async function registerControl(app: FastifyInstance): Promise<void> {
  ...
  app.get("/_control/workspace", async (_req, reply) => {
    ...
    const workspace = wsRow
      ? {
          ...
          name: wsRow.name,
          domain: wsRow.domain,
          avatarUrl: wsRow.avatar_url ?? undefined,
          emojiMap: wsRow.emoji_set ? JSON.parse(wsRow.emoji_set) : {},
        }
      : {
          id: "W001",
          name: "Slack Simulator",
          domain: "slacksim",
          emojiMap: {},
        };
    const users = getAllUsers();

    const channelRows = db.prepare("SELECT * FROM channels").all() as Array<{
      id: string;
      name: string | null;
      type: string;
      archived: number;
  ...
  app.post("/_control/channels/:id/archive", async (req, reply) => {
    ...
    db.prepare("UPDATE channels SET archived = ? WHERE id = ?").run(
      newState,
      id,
    );
  ...
  app.post("/_control/channels/:id/join", async (req, reply) => {
    ...
    const channelTypeCode =
      channel.type === "im" ? "D" : channel.type === "private" ? "G" : "C";
  ...
  app.post("/_control/messages", async (req, reply) => {
    ...
    if (!channelId || !text)
      return reply.status(400).send({ error: "channelId and text required" });
    ...
    const channelTypeCode = !msgChannel
      ? "channel"
      : msgChannel.type === "im"
        ? "im"
        : msgChannel.type === "private"
          ? "group"
          : "channel";
    ...
    if (threadTs) {
      const parentRow = getDb()
        .prepare("SELECT user_id FROM messages WHERE channel_id = ? AND ts = ?")
    ...
    for (const app of apps) {
      if (
        text.includes(`@${app.botUserName}`) ||
        text.includes(`<@${app.botUserId}>`)
      ) {
  ...
  app.post("/_control/messages/:id/reactions", async (req, reply) => {
    ...
    // Find the message by ID or by timestamp (for thread messages)
    const db = getDb();
    let msgRow = db
    ...
    // If not found by ID, try looking up by timestamp (for thread messages)
    if (!msgRow) {
      msgRow = db
        .prepare("SELECT * FROM messages WHERE ts = ?")
        .get(messageId) as typeof msgRow;
    }

    if (!msgRow) return reply.status(404).send({ error: "message not found" });

    // Use the actual message ID from the database row (in case messageId was a timestamp)
    const actualMessageId = msgRow.id;
    const reactionAction = toggleReaction(actualMessageId, name, actingUserId);
    ...
    const eventType =
      reactionAction === "added" ? "reaction_added" : "reaction_removed";
  ...
  app.post("/_control/messages/:id/pin", async (req, reply) => {
    ...
    const wsRow = db.prepare("SELECT id FROM workspace LIMIT 1").get() as
      | { id: string }
      | undefined;
    const teamId = wsRow?.id ?? "W001";
    ...
    const pinCount = (
      db
        .prepare(
          "SELECT COUNT(*) as cnt FROM messages WHERE channel_id = ? AND pinned = 1",
        )
        .get(msgRow.channel_id) as { cnt: number }
    ).cnt;
  ...
  function resolveMentionsInText(raw: string): string {
    ...
    let resolved = raw.replace(/@(\w+)/g, (match, username) => {
      ...
      const bot = db
        .prepare("SELECT bot_user_id FROM apps WHERE bot_user_name = ?")
        .get(username) as { bot_user_id: string } | undefined;
  ...
  app.post("/_control/slash_command", async (req, reply) => {
    const actingUserId = getActingUserId(req);
    const { channelId, command, text } = req.body as {
      channelId: string;
      command: string;
      text?: string;
    };
    ...
    if (!channelId || !command)
      return reply
        .status(400)
        .send({ error: "channelId and command required" });

    const apps = getApps();
    const owningApp = apps.find((a) =>
      a.slashCommands?.some((sc) => sc.command === command),
    );
    if (!owningApp)
      return reply
        .status(404)
        .send({ error: "no app registered for this command" });
  ...
  app.post("/_control/block_action", async (req, reply) => {
    ...
    console.log("[block_action] lookup:", {
      channelId,
      messageTs,
      msgAppId: msgRecord?.appId ?? null,
      bodyAppId: bodyAppId ?? null,
      msgUser: msgRecord?.user ?? null,
    });
    ...
    if (!resolvedAppId && msgRecord) {
      ...
      if (resolvedAppId)
        console.log(
          "[block_action] resolved appId from message author:",
          resolvedAppId,
        );
    }
    ...
    if (!resolvedAppId)
      return reply.status(400).send({
        error: `cannot determine app — msg ${msgRecord ? `found (user=${msgRecord.user}, no app_id)` : "not found"}, bodyAppId=${bodyAppId || "(empty)"}`,
      });
    ...
    const userRow = db
      ...
      .get(actingUserId) as { username: string } | undefined;
    const username = userRow?.username ?? actingUserId;
  ...
  app.post("/_control/modal_block_action", async (req, reply) => {
    const actingUserId = getActingUserId(req);
    const { viewId, blockId, actionId, value, appId, selectedOption } =
      req.body as {
        viewId: string;
        blockId: string;
        actionId: string;
    ...
    try {
      ...
      const tables = tableRows.map(({ name }) => {
        const countRow = rdb
          ...
          .get() as { cnt: number };
        const columns = (
          rdb.prepare(`PRAGMA table_info("${name}")`).all() as Array<{
            cid: number;
            name: string;
            type: string;
            notnull: number;
  ...
  app.get("/_control/db/tables/:name", async (req, reply) => {
    ...
    try {
      ...
      const total = (
        rdb.prepare(`SELECT COUNT(*) as cnt FROM "${name}"`).get() as {
          cnt: number;
        }
      ).cnt;
      const rows = rdb
