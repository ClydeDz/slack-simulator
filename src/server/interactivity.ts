...
interface TriggerEntry { appId: string; expiresAt: number }
...
export function redeemTrigger(triggerId: string): string | null {
  ...
  if (Date.now() > entry.expiresAt) { _triggers.delete(triggerId); return null }
  _triggers.delete(triggerId)
  return entry.appId
}

// ── Modal store (viewId → { view, appId }) ────────────────────────────────────
interface ModalEntry { view: ModalView; appId: string }
...
export async function dispatchViewSubmission(params: {
  ...
  if (params.app.socketModeEnabled) {
    const { enqueueSocketModeInteractive } = await import('./socketModeServer')
    const envelopeId = `Ev${Date.now()}${Math.random().toString(36).slice(2, 8)}`
    const result = await enqueueSocketModeInteractive(params.app.id, envelopeId, payload)
    return result.responsePayload as { response_action?: string; view?: unknown } | null
  } else if (params.app.requestUrl) {
...
// ── views.open ────────────────────────────────────────────────────────────────
export function openModal(rawView: Record<string, unknown>, appId: string): ModalView {
...
// ── views.update ──────────────────────────────────────────────────────────────
export function updateModal(viewId: string, rawView: Record<string, unknown>): ModalView | null {
...
// ── views.push ────────────────────────────────────────────────────────────────
export function pushModal(rawView: Record<string, unknown>, appId: string): ModalView {
  const viewId = `V${Date.now()}${Math.random().toString(36).slice(2, 8)}`
  const view: ModalView = {
    id: viewId,
    type: 'modal',
